...
input_dir <- "/Users/saudat/Desktop/bed_files/final_bed_files/cinte_files/converted/general_peaks//"
output_dir <- "/Users/saudat/Desktop/bed_files/final_bed_files/cinte_files/converted/general_peaks/genic_regions/"
...
setwd("/Users/saudat/Desktop/bed_files/final_bed_files/cinte_files/converted/general_peaks/")
...
cleanPeaksBasedOnAncestry<-function (crobu_peaks,cinte_peaks, ancestor_peaks) {
  ...
  filtered_peaks = ancestor_peaks[ancestor_peaks@elementMetadata$name  %in% cinte_peaks@elementMetadata$name & ancestor_peaks@elementMetadata$name  %in% crobu_peaks@elementMetadata$name]
  cinte_peaks = cinte_peaks[cinte_peaks@elementMetadata$name  %in% filtered_peaks@elementMetadata$name]
  crobu_peaks = crobu_peaks[crobu_peaks@elementMetadata$name  %in% filtered_peaks@elementMetadata$name]
  ancestor_peaks = ancestor_peaks[ancestor_peaks@elementMetadata$name  %in% filtered_peaks@elementMetadata$name]
  assign("common_peaks", filtered_peaks, envir = .GlobalEnv)
  assign("cinte_peaks", cinte_peaks, envir = .GlobalEnv)
  assign("crobu_peaks", crobu_peaks, envir = .GlobalEnv)
  assign("ancestor_peaks", ancestor_peaks, envir = .GlobalEnv)
...
for (file in bed_files) {
  ...
  peaks <- peaks[peaks@elementMetadata$name %in% featpeaks$CDS |
                   peaks@elementMetadata$name %in% featpeaks$intron]
  ...
  export.bed(peaks, con = paste0(output_dir, core_name, "cinte_FilteredGenicRegions.bed"))
  
  peaks <- crobu_peaks[!duplicated(crobu_peaks@elementMetadata$name)] #crobu
  export.bed(peaks, con = paste0(output_dir, core_name, "crobu_FilteredGenicRegions.bed"))
  
  peaks <- ancestor_peaks[!duplicated(ancestor_peaks@elementMetadata$name)] #crobu
  export.bed(peaks, con = paste0(output_dir, core_name, "anc_FilteredGenicRegions.bed"))
  
  # Append results to the table
  results_table <- rbind(results_table, data.frame(
    File = file,
    Step1 = step1_count, #filter GENIC
