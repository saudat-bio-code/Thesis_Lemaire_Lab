...
genNullSeqs_ranges <- function (inputBedFN, genomeVersion = "hg19", outputBedFN = "negSet.bed", 
          ...
          genome = NULL) 
{
  if (requireNamespace("GenomicRanges", quietly = TRUE) & requireNamespace("rtracklayer", 
                                                                           quietly = TRUE) & requireNamespace("BiocGenerics", quietly = TRUE) & 
      requireNamespace("Biostrings", quietly = TRUE) & requireNamespace("GenomeInfoDb", 
                                                                        quietly = TRUE) & requireNamespace("IRanges", quietly = TRUE) & 
      requireNamespace("S4Vectors", quietly = TRUE)) {
    if (is.null(genome)) {
      if (toupper(genomeVersion) == "HG18") {
        if (requireNamespace("BSgenome.Hsapiens.UCSC.hg18.masked", 
                             quietly = TRUE)) {
    ...
    matchSeqs = function(gc1, gc2, len1, len2, rpt1, rpt2, 
    ...
    outbed = gsub(" ", "", outbed)
    utils::write.table(as.matrix(outbed[, 1:3]), quote = FALSE, 
    ...
    if (requireNamespace("seqinr", quietly = TRUE)) {
      ...
      inseqnams = paste(as.character(inbed[, 1]), inbed[, 
                                                        2], inbed[, 3], "pos", 1:nrow(inbed), sep = "_")
...
<bytecode: 0x7ee14ae0>
  ...
  ichr = "KhS1735"
  seq = genhg[[ichr]]
  #rpt = Biostrings::masks(genhg@)[["TRF"]]
  
  rpt=GRanges(seqnames= ichr)
  trf[seqnames(trf) == ichr]
  ...
  function(bed) {
    ...
    for (ichr in as.character(chrs)) {
      ...
      if (length(jj) > 0) {
        ...
        jbed = bed[jj]
        jrpts = rep(0, length(jj))
        olaps <- IRanges::findOverlaps(rpt@ranges, jbed@ranges)
        qdf = GenomicRanges::as.data.frame(rpt)[S4Vectors::queryHits(olaps),
                                                ]
        isect <- IRanges::pintersect(IRanges::IRanges(start = qdf$start,
                                                      end = qdf$end), jbed@ranges[S4Vectors::subjectHits(olaps)])
